...
public class Cat implements Rival {
    ...
    public Cat(String name, double jump, double run) {
        this.name = name;
        this.jump = jump;
        this.run = run;
    }
    ...
    public void makeActive(Obstacle obstruction){
        ...
        if (obstruction.getType().equals(typeTreadmill)){
            if (!run(obstruction.getSize())){
                System.out.println("Кот " + this.name + " выбывает !");
                result = false;
            }
        }
    ...
    private boolean run (double size) {
        ...
        if (size < this.run){
            System.out.println("Кот " + this.name + " пробежал: " + size);
            result = true;
        } else {
            System.out.println("Кот " + this.name + " не может пробежать: " + size + ". Предел: " + this.run);
        }
        return result;
    }
    ...
    private boolean jump (double size) {
        boolean result = false;
        if (size < this.jump){
            System.out.println("Кот " + this.name + " перепрыгнул: " + size);
            result = true;
        } else {
            System.out.println("Кот " + this.name + " не может перепрыгнуть: " + size + ". Предел: " + this.jump);
        }
        return result;
    }
}
